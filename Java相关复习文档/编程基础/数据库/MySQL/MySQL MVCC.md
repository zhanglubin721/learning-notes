...
![在这里插入图片描述](image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBATm9ibGVnYXNlc2dvbw==,size_19,color_FFFFFF,t_70,g_se,x_16.png)
...
![在这里插入图片描述](image/watermark,type_d3F5LXplbmhlaQ,shadow_50,text_Q1NETiBATm9ibGVnYXNlc2dvbw==,size_20,color_FFFFFF,t_70,g_se,x_16-20220621150811013.png)



解释：

**当前读：**即读取最新的数据行版本，无视是否有其他事务在对这行数据进行修改。这种读模式常用于 SELECT ... LOCK IN SHARE MODE（共享锁）或者 SELECT ... FOR UPDATE（排他锁）的查询操作中。
**快照读：**是 MVCC（多版本并发控制）的核心所在，每个事务读取的是对应快照版本的数据，并非最新的数据。快照读不会阻止其他的事务对这行数据的修改。
为了找到合适的行记录版本，InnoDB 会获取事务开始时的事务ID（也被称作 read-view），以确保在该事务生命周期内都读取到相同版本的数据，保证数据的一致性。
对于每一行数据，InnoDB 将记录两个系统版本号：

1. DB_TRX_ID：修改该行数据的事务ID。
2. DB_ROLL_PTR：用来获取该行数据的上一个版本信息。



1. up_limit_id: 这是当前读事务视图中可见的最大事务ID。换句话说，ID小于该值的所有事务都被认为是已经提交的事务，因此其所做的改变应该在这个读事务视图中可见。
2. low_limit_id: 这是在当前读事务视图中可见的最小事务ID。任何大于或等于此ID的事务都被认为是在当前读事务开始后开始的，因此它们所做的改变在这个读事务视图中不可见。
3. trx_list: 这是在当前读事务开始时正在活动的所有事务ID的列表。对于此列表中的任何事务ID，它们所做的改变在当前读事务视图中是不可见的，除非那些事务已经提交。

（up_limit_id < low_limit_id，这点很重要，不要被up和low这两个单词迷惑）

当进行快照读时，会取得 read-view 信息，其中包含 up_limit_id，low_limit_id 和 trx_list。接着，将当前读取的行记录的 DB_TRX_ID 与这些信息进行比较，以确定该行数据在当前事务中应该看到哪个版本。
具体规则如下：

- 如果 DB_TRX_ID < up_limit_id，则该版本已经被一个已提交的事务修改，可见。
- 如果 up_limit_id <= DB_TRX_ID < low_limit_id，则正在被 trx_list 中的某一未提交的事务修改，不可见。
- 如果 low_limit_id <= DB_TRX_ID，则正在被一个尚未开始的事务修改，可见。
